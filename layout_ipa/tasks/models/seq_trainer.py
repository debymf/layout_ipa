...
LAYOUT_LM = settings["layout_lm_base"]


class SeqTransformerTrainer(Task):
    def __init__(self, **kwargs):
        super(SeqTransformerTrainer, self).__init__(**kwargs)
    ...
    def run(
        ...
        if mode == "train":
            ...
            layout_lm_config = AutoConfig.from_pretrained(
                LAYOUT_LM, num_labels=num_labels
            )
            model_layout = AutoModel.from_pretrained(LAYOUT_LM, config=layout_lm_config)

            model_layout = model_layout.to(device)
            model_instruction = model_instruction.to(device)
    ...
    def train(
        ...
        for epoch in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                output_instruction = model_instruction(**inputs_instruction)
                output_layout = model_layout(**inputs_ui)
    ...
    def eval(
        ...
        if eval_fn is not None:
            score = eval_fn(preds=preds, gold=out_label_ids)
            if mode == "test":
                out_preds = {
                    "index": all_index.tolist(),
                    "preds": preds.tolist(),
                    "gold": out_label_ids.tolist(),
                }
                with open(
                    f"./cache/output/bin_preds_{bert_model.replace('/', '_')}.json", "w"
                ) as fp:
