...
class LayoutLMAndBertSimple(PreTrainedModel):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.mlp_screen = MLP(768 * 5, 768)
        self.mlp_ui_element = MLP(768, 768)
        self.mlp_combine = MLP(768 * 2, 128)
    ...
    def forward(self, input_close_elements, input_ui):
        ...
        screen_embedding = self.mlp_screen(output_close_elements)

        output_ui_model = self.model_ui(**input_ui)
        ui_embedding = output_ui_model[1]
        ui_embedding = self.mlp_ui_element(ui_embedding)

        output_combined = self.mlp_combine(
