...
class BiDAF(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.char_conv = nn.Sequential(
            nn.Conv2d(
                1, args.char_channel_size, (args.char_dim, args.char_channel_width)
            ),
            nn.ReLU(),
        )
        ...
        # highway network
        assert self.args.hidden_size * 2 == (
            self.args.char_channel_size + self.args.word_dim
        )
        for i in range(2):
            setattr(
                self,
                ...
                nn.Sequential(
                    Linear(args.hidden_size * 2, args.hidden_size * 2), nn.ReLU()
                ),
            )
            setattr(
                self,
                ...
                nn.Sequential(
                    Linear(args.hidden_size * 2, args.hidden_size * 2), nn.Sigmoid()
                ),
            )
